...
pdf(paste0("hist/Mod_pred_", seed, "_2m.pdf"),width=6,height=4,paper='special')
ggplot(x, aes(x=x[,1])) + 
    geom_histogram() +
...
pdf(paste0("hist/Edges_", seed, "_2m.pdf"),width=6,height=4,paper='special')
hist(x[,2])
dev.off()

x <- read.table(paste0("hist/", seed, "_adj_2m.txt"), header = FALSE, sep = "\t")
x <- x[1:nodes, 1:nodes]
...
pdf(paste0("hist/Meanadj_", seed, "_2m.pdf"),paper='special')
