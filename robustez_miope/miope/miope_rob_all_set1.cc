...
int main()
{
  ...
  ofstream ocon;
  bas.open_ofstream(ocon, "miope_rob/" +  bas.inttostring(seed) + "_con_set1.txt");
  ocon << "Initial" << "\t" << "Final" << endl;
    
  ofstream omod;
  bas.open_ofstream(omod, "miope_rob/" + bas.inttostring(seed) + "_mod_set1.txt");
  omod << "Initial" << "\t" << "Final" << endl;
  
  ofstream orob;
  bas.open_ofstream(orob, "miope_rob/" + bas.inttostring(seed) + "_rob_set1.txt");
  orob << "Initial" << "\t" << "Final" << endl;
  
  ofstream otime;
  bas.open_ofstream(otime, "miope_rob/" + bas.inttostring(seed) + "_time_set1.txt");
  ...
  for(int k = 0; k < num_exp; k++){
    ...
    while(!f_maxmut){
        f_aum = false;
        count = 0;
        vac.robustness_1mutation_gaps(s, mgood, mbad, oc, ot, nodes, modules, predpar, intmods, nugaps, last_rob);
        ...
        while(!f_aum && !f_maxmut){
            ...
            if(fifu.cap_to_s_gaps(nodes, mgood, mbad, oc, ot, modules, intmods, nugaps)){
                new_one.robustness_1mutation_gaps(s, mgood, mbad, oc, ot, nodes, modules, predpar, intmods, nugaps, new_rob);
                if(new_rob > last_rob){
                    vac.clear();
                    vac.copy(new_one);
                    f_aum = true;
