...
Mod <- x[1:nu_exp,1]
Rob_1m <- read.table(paste0("rob/", seed, "_allmut_set1.txt"), header = TRUE, sep = "\t")
...
t1 <- cor.test(data_p$Mod_pred, data_p$Rob_prom,  method = "pearson")
prprom <- ggplot(data_p, aes(x = Mod_pred, y = Rob_prom)) +
    geom_point(alpha = 0.25) +
    labs(caption = paste0("n = ", nrow(data_p),"; p_value = ", t1$p.value,"; r = ", t1$estimate)) +
   theme(plot.caption = element_text(size = 8))
...
pdf(paste0("rob/Allmut_", seed, "_set1.pdf"),paper='special')
    grid.arrange(prob, prprom, pdist, pz)
dev.off()
...
pdf(paste0("rob/Gan_", seed, "_set1.pdf"),paper='special')
...
pdf(paste0("rob/Per_", seed, "_set1.pdf"),paper='special')
