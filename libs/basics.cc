...
{
	char s[100];
	sprintf(s, "dot -Tpdf %s.dot -o %s.pdf", arch.c_str(), arch.c_str());
	system(s);
}
    ...
    if (ext=="pdf")
    ...
    else 
     {
      if (ext=="eps")
       {
        char s[100];
        sprintf(s, "dot -Tps2 %s.dot -o %s.eps", arch.c_str(), arch.c_str());
        system(s);
       }
      else
       {
        if (ext=="png")
         {
          char s[100];
          sprintf(s, "dot -Tpng %s.dot -o %s.png", arch.c_str(), arch.c_str());
          system(s);
         }
        else 
         {
...
void Basics::get_circo_dot_fig(string arch)
{
	char s[100];
	sprintf(s, "circo -Tpdf %s.dot -o %s.pdf", arch.c_str(), arch.c_str());
	system(s);
}

void Basics::get_circo_dot_fig(string arch, string ext)
{
    if (ext=="pdf")
      get_circo_dot_fig(arch);
    else 
     {
      if (ext=="eps")
       {
        char s[100];
        sprintf(s, "circo -Tps2 %s.dot -o %s.eps", arch.c_str(), arch.c_str());
        system(s);
       }
      else
       {
        if (ext=="png")
         {
          char s[100];
          sprintf(s, "circo -Tpng %s.dot -o %s.png", arch.c_str(), arch.c_str());
          system(s);
         }
        else 
         {
          cout << "[Error]: Basics::get_circo_dot_fig does not recognize extension " << ext << ".\n";
          exit(1);
         }
       }
     }	
