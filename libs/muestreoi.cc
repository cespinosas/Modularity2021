...
MuestreoI::MuestreoI(Alea& jacta)
{
  start_rng(jacta);
  yaparamsmu = false;
  yapart_mues = false;
...
void MuestreoI::start_rng(Alea& jacta)
{
  est = jacta;
  basic.start_rng(est);
}

void MuestreoI::set_params_muestreo(int samplesizep, int nugenp, int stepsizep, int conectividadp, int modulesp)
{
...
void MuestreoI::set_partition(set<set<int> > &predparp)
{
    if(modules != (int)predparp.size()){
      cout << "[Error]: Number of modules and modules in given partition are not the same in MuestreoI::set_partition.\n";
      exit(1);
    }
...
void MuestreoI::print_params_mues(ostream& sal)
{
  sal << "Integer_weights\n";
...
int MuestreoI::make_modular_nw_unos_mues(GraphI &red, int *s, int nodes)
{
  if(!yapart_mues){
    cout << "[Error]: Partition was not initialized when  MuestreoI::make_modular_nw_mues was called.\n";
    exit(1);
  }
    ...
    for (int i = 0; i < modules; i++) {
        ...
        if(whi.count(n)!=0){
            for (ite = whi.begin(); ite != whi.end(); ite++){
                if(*ite >= nodes) {
                    cout << "[Error]: nodes in partition does not correspond to nodes in network in MuestreoI::make_modular_nw.\n";
                    exit(1);
                }
            if(s[n]*s[*ite] == 1)
...
void MuestreoI::complete_conectiviness(GraphI& vacia, int *s, int nodes, int& nu_unos)
{
    int lugar_i, lugar_j;
    int valor;
    int ed;
...
void MuestreoI::muestreo_rn_v2_onenw(GraphI &vacia, int *s, int nodes, int &nu_unos, GraphI &regreso, int* old_mgood, int* old_mbad, int** old_oc, int** old_ot)
{  
//     cout << nu_unos << endl;
    int path = stepsize;
    bool law;
...
void MuestreoI::muestreo_rn_v2_onenw_gaps(GraphI &vacia, int *s, int nodes, int &nu_unos, GraphI &regreso, int* old_mgood, int* old_mbad, int** old_oc, int** old_ot, int ***intmods, int nugaps)
{  
//     cout << nu_unos << endl;
    int path = stepsize;
    bool law;
...
void MuestreoI::start_gaps_ics(int **gaps, int nugaps, int modules, int nodes, int** modgapsp)
{
...
void MuestreoI::start_all_ics(int **alls, int nuic, int modules, int nodes)
{
  int** s;
  basic.create_array(s, 2, nodes);
  for (int i = 0; i < nodes; i++) {
